...
public List<TipoAccion> listarTipoAccionAfectaPlanilla(Cias cias, String rol)
{
    ...
    List<TipoAccion> listaTipo = (List<TipoAccion>) em.createNativeQuery(query.toString(), TipoAccion.class).setParameter(1, cias.getCodCia()).setParameter(2, rol).getResultList();
    return listaTipo != null ? listaTipo : new ArrayList<TipoAccion>();
}
...
{
    List<TipoAccion> listaTipo = em.createQuery("SELECT t FROM TipoAccion t WHERE t.tipoAccionPK.codCia = :codCia AND t.afectaSal = 'N' AND t.estado = 'A' and t.retiro=1", TipoAccion.class).setParameter("codCia", cias.getCodCia()).getResultList();
    return listaTipo != null ? listaTipo : new ArrayList<TipoAccion>();
}
}
