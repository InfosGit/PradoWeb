...
private List<ProgramacionPla> listaProgramacionPla = new ArrayList<ProgramacionPla>();  

@PostConstruct
public void init()
{
    setEmpresa(getSessionBeanEMP().getCompania());
}
...
public List<ProgramacionPla> getListaProgramacionPla()
{
    return listaProgramacionPla;
}

public void setListaProgramacionPla(List<ProgramacionPla> listaProgramacionPla)
{
...
public String listProgramacion(){
     ...
     if (tipoPlanilla == null && tipoPlanilla != -1){
     addMessage("Mensaje", "Seleccione el tipo de planilla", TipoMensaje.INFORMACION);
     }
     else{
    listaProgramacionPla = planillaSessionBean.getProgramacionPlaByTipo(getEmpresa().getCodCia(), tipoPlanilla);    
     }
    return null;
