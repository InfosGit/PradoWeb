...
@ViewScoped
public class PuestosBackendBean extends AbstractJSFPage implements Serializable
{
...
public void _init()
{
    setListaFuncionPuesto(reclutamientoFacade.findFuncionPuestoByEmpresa(getSessionBeanEMP().getCompania()));
    setListaPerfil(reclutamientoFacade.findPerfilByEmpresa(getSessionBeanEMP().getCompania()));
}
...
{
    listaCriterios = reclutamientoFacade.getListaCriteriosByCias(getSessionBeanEMP().getCompania().getCodCia());
...
{
    listaAreas = reclutamientoFacade.findAreasStaffByCias(getSessionBeanEMP().getCompania());
    return listaAreas;
}
...
{
    listaDepartamentos = reclutamientoFacade.findDepartamentosByCias(getSessionBeanEMP().getCompania());
...
{
    listaTipoPuesto = reclutamientoFacade.getTipoPuestosByEmpresa(getSessionBeanEMP().getCompania());
...
{
    listaPuestos = reclutamientoFacade.getPuestosByEmpresa(getSessionBeanEMP().getCompania());
    return listaPuestos;
}
...
public String guardar$crud$action()
{
    ...
    nuevoPuesto.setTipoPuesto((tipoPuesto != null && tipoPuesto != -1) ? new TipoPuesto(new TipoPuestoPK(getSessionBeanEMP().getCompania().getCodCia(), tipoPuesto)) : null);
    nuevoPuesto.setStatus(estado);
    nuevoPuesto.setDepartamentos((departamento != null && departamento != -1) ? new Departamentos(new DepartamentosPK(getSessionBeanEMP().getCompania().getCodCia(), departamento)) : null);
    nuevoPuesto.setAreasStaff((area != null && area != -1) ? new AreasStaff(new AreasStaffPK(getSessionBeanEMP().getCompania().getCodCia(), area)) : null);
    ...
    try
        ...
        if (getEstadoAccion() == null || getEstadoAccion().equals(0))
            {
            reclutamientoFacade.guardarPuesto(nuevoPuesto, getSessionBeanEMP().getCompania());
            limpiarCampos();
            }
...
public String editar$crud$action()
{
    ...
    setDepartamento(getPuestoSeleccionado().getDepartamentos() != null ? getPuestoSeleccionado().getDepartamentos().getDepartamentosPK().getCodDepto() : -1);
...
public String guardarCriterio$crud$action()
{
    ...
    CriteriosXPuestoPK pk = new CriteriosXPuestoPK(getSessionBeanEMP().getCompania().getCodCia(), getPuestoSeleccionado().getPuestosPK().getCodPuesto(), getCriterioSeleccionado().getCriterioPK().getCodigo());
