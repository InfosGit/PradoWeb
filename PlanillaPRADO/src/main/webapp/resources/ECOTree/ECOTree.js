...
ECOTree = function (obj, elm)
{
    this.config =
    {
        ...
        expandedImage : '/PlanillaWebPapelera/resources/ECOTree/img/less.gif',
        collapsedImage : '/PlanillaWebPapelera/resources/ECOTree/img/plus.gif',
        transImage : '/PlanillaWebPapelera/resources/ECOTree/img/trans.gif'
    }
...
ECOTree._getAutoRenderMode = function()
{
    ...
    //var is_chr = /Chrome/i.test(navigator.userAgent);
        ...
        if (!node._isAncestorCollapsed())
        {
            switch (this.render)
            {
                case "CANVAS":
                    ...
                    {
                        s.push('<a id="t' + node.id + '" href="' + node.target + '" onclick="nodo' +  node.id + '.className=\'noOculto\';">' + node.dsc + '</a>');
                    }
                    else
                    {
                        s.push(node.dsc);
...
ECOTree.prototype.toString = function ()
{
    ...
    {
        case "CANVAS":
            s.push('<canvas id="ECOTreecanvas" width="7500" height="7500"/>');
            break;
...
ECOTree.prototype.add = function (id, pid, dsc, w, h, c, bc, target, meta)
{
    ...
    var h_ = pnode.nodeChildren.length; //Add it as child of it's parent
    node.siblingIndex = h_;
    pnode.nodeChildren[h_] = node;
}
