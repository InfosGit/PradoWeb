...
{
    accion = CREANDO;
    tipoEvaluacionTableModel = empleadosBean.listarTiposEvaluacion(getSessionBeanEMP().getCompania());
}
...
private void guardarCampania()
{
    try
        ...
        campaniaPK.setCodCia(getSessionBeanEMP().getCompania().getCodCia());
        campaniaPK.setPeriodo(periodo);
        campaniaPK.setCodCampania(empleadosBean.maxCampania(getSessionBeanEMP().getCompania()).shortValue());
...
{
    listaPreevaluaciones = empleadosBean.findPreevaluacionByCias(getSessionBeanEMP().getCompania());
