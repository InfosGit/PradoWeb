...
public MenuModel getMenuModel()
{
    ...
    if (isInRole("rrhh"))
        {
        n = menuFacade.findAllEnOrden(getSessionBeanEMP().getCompania(), "2");
        m = menuFacade.findAllSubMenu(getSessionBeanEMP().getCompania(), "2");
        }
    else if (isInRole("empleados"))
        {
        n = menuFacade.findAllEnOrden(getSessionBeanEMP().getCompania(), "1");
        m = menuFacade.findAllSubMenu(getSessionBeanEMP().getCompania(), "1");
        }
    else if (isInRole("jefes"))
        {
        n = menuFacade.findAllEnOrden(getSessionBeanEMP().getCompania(), "3");
        m = menuFacade.findAllSubMenu(getSessionBeanEMP().getCompania(), "3");
        }
