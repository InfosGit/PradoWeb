...
// ==
private transient AutocompleteProgramacionPlaConverter planillaConverter;

@PostConstruct
public void init()
{
    listaTiposPlanilla = planillaSessionBean.listarTipos(getSessionBeanEMP().getCompania());
    listaDeptos = planillaSessionBean.findDepartamentos(getSessionBeanEMP().getCompania());
}
...
public AutocompleteProgramacionPlaConverter getPlanillaConverter()
{
    if (tipoPlanilla != null && tipoPlanilla != -1)
        {
        planillaConverter = new AutocompleteProgramacionPlaConverter(planillaSessionBean.getProgramacionPlaByTipo(getSessionBeanEMP().getCompania().getCodCia(), tipoPlanilla));
        }
    else
        {
...
public String enviar$correo$action()
{
    ...
    try
        ...
        if (codDepto.equals(new Short("0")))
            ...
            {
            Departamentos d = planillaSessionBean.findDeptoById(new DepartamentosPK(getSessionBeanEMP().getCompania().getCodCia(), codDepto));
