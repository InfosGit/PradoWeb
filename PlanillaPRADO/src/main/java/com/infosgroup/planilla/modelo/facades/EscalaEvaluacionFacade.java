...
public String getEscalaByEvaluacion(Evaluacion ev, Integer valor)
{
    ...
    e = em.createNativeQuery("select * from escala_evaluacion where cod_cia = ? and cod_tipo_evaluacion = ? and " + valor + " between rango_inicial and rango_final", EscalaEvaluacion.class).setParameter(1, ev.getEvaluacionPK().getCodCia()).setParameter(2, ev.getTipoEvaluacion1().getTipoEvaluacionPK().getCodTipoEvaluacion()).getResultList();
    if (e != null && e.size() > 0)
        {
