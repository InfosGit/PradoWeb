...
public void init()
{
    ...
    setListaDepartamentos(planillaSessionBean.findDepartamentos(getSessionBeanEMP().getCompania()));
    setListaPuestos(planillaSessionBean.findPuestos(getSessionBeanEMP().getCompania()));
...
{
    setListaPreEvaluacion(empleadosBean.findPreevaluacionByCias(getSessionBeanEMP().getCompania()));
...
public String definirEvaluaciones$action()
{
    ...
    try
        ...
        for (Evaluador e : evaluadores)
            ...
            planillaSessionBean.eliminarEvaluadorEvaluaciones(e.getPreEvaluacion(), e.getEmpleados(), evaluacion);
...
//                } else {
            ...
            empleado.getEvaluacionList().clear();
            empleado.getEvaluacionList().addAll(evaluacion);
            logger.log(Level.SEVERE, "Se agregaron todos a {0}", new Object[]
                        {
                        e.getEmpleados().getNombreCompleto()
                        });
//                }
...
public String buscarEvaluadoresPorNombre()
{
    setFiltroBusqueda("N");
    setModoBusqueda(Boolean.TRUE);
    castListaEmpleadosToEvaluador(planillaSessionBean.listaEmpleados(getSessionBeanEMP().getCompania()));
    return null;
}
...
{
    castListaEmpleadosToEvaluador(planillaSessionBean.findEmpleadosByDepartamentos(new Departamentos(new DepartamentosPK(getSessionBeanEMP().getCompania().getCodCia(), departamento))));
}

public void listarEmpleadosByPuesto(AjaxBehaviorEvent event)
{
    castListaEmpleadosToEvaluador(planillaSessionBean.findEmpleadosByPuestos(new Puestos(new PuestosPK(getSessionBeanEMP().getCompania().getCodCia(), puesto))));
}
...
public String calcularEvaluados()
{
    ...
    if (evaluadorSeleccionado.getCriterioEvaluacion().equals("D"))
        ...
        evaluadorSeleccionado.setEvaluadoList(planillaSessionBean.findEvaluadosByDepartamentos(evaluadorSeleccionado, new Departamentos(new DepartamentosPK(getSessionBeanEMP().getCompania().getCodCia(), evaluadorSeleccionado.getDepartamento()))));
...
public String wizardFlowListener(FlowEvent flowEvt)
{
    ...
    switch (a)
        ...
        case 1:
            ...
            setEmpleados(planillaSessionBean.listaEmpleados(getSessionBeanEMP().getCompania()));
            break;
        case 2:
            setEvaluadores(empleadosBean.findEvaluadoresByPreEvaluacion(getSessionBeanEMP().getPreEvaluacionSeleccionada()));
...
public String limpiarSeleccion()
{
    ...
    setEmpleados(empleadosBean.findEmpleadosByCias(getSessionBeanEMP().getCompania()));
    return null;
}
