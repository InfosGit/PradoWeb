...
import com.infosgroup.planilla.modelo.facades.DocumentosCandidatoFacade;
...
@EJB
private DocumentosCandidatoFacade documentosCandidatoFacade;
...
// ===========================================================================================================
private static final String rutaCarpetaAdjuntos = "/root/Escritorio/DF/prueba/";
private String nombreArchivoAdjunto;
private String rutaArchivoAdjunto;
private String descripcion;
private List<String> listaArchivosAdjuntos;
...
private List<DocumentosCandidato> listDocumentosCandidato;
private DocumentosCandidato documentoSeleccionado;
// =======================================
// ===========SUBIR ARCHIVOS==============
private String destino = "/root/Escritorio/DF/prueba";
private String archivo;


public List<String> getListaArchivosAdjuntos()
{
    return listaArchivosAdjuntos;
}

public void setListaArchivosAdjuntos(List<String> listaArchivosAdjuntos)
{
...
public DocumentosCandidato getDocumentoSeleccionado()
{
    return documentoSeleccionado;
}

public void setDocumentoSeleccionado(DocumentosCandidato documentoSeleccionado)
{
    this.documentoSeleccionado = documentoSeleccionado;
}

public List<DocumentosCandidato> getListDocumentosCandidato()
{
    return listDocumentosCandidato;
}

public void setListDocumentosCandidato(List<DocumentosCandidato> listDocumentosCandidato)
{
    this.listDocumentosCandidato = listDocumentosCandidato;
}
...
public void init()
{
    ...
    listaArchivosAdjuntos = new ArrayList<String>();
    ...
    //el = sessionBeanParametros.findDepartamentosByPais(sessionBeanParametros.findPaisesByid(new Short("4")));
    ...
    candidatosListModel = reclutamientoFacade.getCandidatosByEmpresa(getSessionBeanEMP().getCompania());
...
public void cargarArchivo$fileUploadListener(FileUploadEvent event)
{
    try
        {
        UploadedFile file = event.getFile();
        Logger.getLogger(getClass().getCanonicalName()).log(Level.INFO, "Archivo [{0}]", file);
        nombreArchivoAdjunto = file.getFileName().replace(" ", "_");
        rutaArchivoAdjunto = rutaCarpetaAdjuntos + Calendar.getInstance().getTimeInMillis() + "---" + nombreArchivoAdjunto;
        File archivoServidor = new File(rutaArchivoAdjunto);
        Logger.getLogger(getClass().getCanonicalName()).log(Level.INFO, "Cargando [{0}]", archivoServidor.getAbsolutePath());
        FileOutputStream fos = new FileOutputStream(archivoServidor);
        fos.write(file.getContents());
        fos.flush();
        fos.close();
        
        DocumentosCandidatoPK adjuntoPK = new DocumentosCandidatoPK();        
        adjuntoPK.setCodCia(candidatoSeleccionado.getCandidatoPK().getCodCia());
        adjuntoPK.setCodCandidato(candidatoSeleccionado.getCandidatoPK().getCodCandidato());
        adjuntoPK.setCodArchivo(documentosCandidatoFacade.max(candidatoSeleccionado)+1L);
        listaArchivosAdjuntos.add(rutaArchivoAdjunto);
        addMessage("CRM", "Archivo cargado exitosamente", TipoMensaje.INFORMACION);
        }
    catch (Exception ex)
        {
        addMessage("CRM", "No se puede cargar el archivo o no existe", TipoMensaje.ERROR);
        //Logger.getLogger(CargaDatosManagedBean.class.getName()).log(Level.SEVERE, null, ex);
        }
}

public String adjuntarArchivo$action()
{
//    if (rutaArchivoAdjunto == null)
    if ((listaArchivosAdjuntos == null) || listaArchivosAdjuntos.isEmpty())
        {
        addMessage("Infosweb CRM", "Seleccione archivos/documentos y carguelos al servidor", TipoMensaje.ADVERTENCIA);
        return null;
        }
    for (String ruta : listaArchivosAdjuntos)
        {
        Logger.getLogger(getClass().getCanonicalName()).log(Level.INFO, "Adjuntando [{0}]", ruta);

        int i = nombreArchivoAdjunto.lastIndexOf('.');
        String extension = (i != -1) ? nombreArchivoAdjunto.substring(i + 1).toLowerCase() : "none";
        ...
        DocumentosCandidato adjunto = new DocumentosCandidato();
        adjunto.setDocumentosCandidatoPK(adjuntoPK);
        adjunto.setRuta(ruta);
        adjunto.setDescripcion(descripcion);
        adjunto.setFechaCargo(Calendar.getInstance().getTime());
        adjunto.setExtension(extension);
        documentosCandidatoFacade.create(adjunto);
        addMessage("Infosweb CRM", "Archivos/Documentos guardados exitosamente", TipoMensaje.INFORMACION);
        }
    descripcion = null;
    listDocumentosCandidato = documentosCandidatoFacade.findByCandidato(candidatoSeleccionado);
    return null;
}
...
public String cancelarCargaArchivo$action()
{
    descripcion = null;
    return null;
}
...
public String entrevistas$agregar$action()
{
    try
        ...
        e.setTipoEntrevistador(entrevistas$tipoEntrevistador);
        if (entrevistas$tipoEntrevistador == 1)
            {
...
public String editar$action()
{
    try
        ...
        generales$etnia = c.getEtnia();
        // ==
        generales$dui = c.getNumDui();
        generales$isss = c.getNumIsss();
        generales$nit = c.getNumNit();
        generales$fechaExpDui = c.getFechaExpDui();

        if (c.getCodPaisNacionalidad() != null)
            {
            pais = sessionBeanParametros.findPaisesByid(c.getCodPaisNacimiento());
            deptosExpDUISelectItemListModel = sessionBeanParametros.findDepartamentosByPais(pais);
            }
        generales$departamentoExpDui = generales$paisNacionalidad + ":" + c.getExpedicionDui();

        if (c.getExpedicionDui() != null)
            {
            depto = sessionBeanParametros.findDepartamentoById(new DeptosPK(new Short(generales$paisNacionalidad), new Short(c.getExpedicionDui())));
        ...
        for (NivelesXCandidato n : ln)
            ...
            p.setProfesion(sessionBeanParametros.findProfesionById(new ProfesionPK(n.getNivelesXCandidatoPK().getCodCia(), n.getSubNivel())));
        ...
        for (EmergenciaXCandidato e : le)
            ...
            p.setParentesco(sessionBeanParametros.findParentescoById(new ParentescoPK(getSessionBeanEMP().getCompania().getCodCia(), e.getCodParentesco())));
        ...
        // ==
        experienciasLaboralesCandidato = new ArrayList<ExperienciaLaboralCandidato>();
        List<ExpLaboralCandidato> lelc = reclutamientoFacade.findExperienciasLaboralesByCandidato(c);
        for (ExpLaboralCandidato elc : lelc)
            {
            ExperienciaLaboralCandidato e = new ExperienciaLaboralCandidato();
            e.setLugar(elc.getLugarTrabajo());
            e.setPuesto(elc.getPuestos());
            e.setFechaInicio(elc.getFechaInicio());
            e.setFechaFin(elc.getFechaFin());
            e.setMotivoRetiro(elc.getMotivoRetiro());
        ...
        for (EntrevistaXCandidato entr : lentr)
            ...
            ec.setEntrevistador(entr.getEntrevistador());
        ...
        /*
         * nombre = c.getNombre(); apellido = c.getApellido(); apellidoCasada = c.getApCasada(); fechaSolicitud = c.getFecSolicitud(); sexo = c.getSexo(); estadoCivil = c.getEstadoCivil(); // ==
         * generales$pais = "" + c.getCodPaisDomic(); pais = sessionBeanParametros.findPaisesByid(c.getCodPaisDomic()); deptosDomicilioSelectItemListModel =
         * sessionBeanParametros.findDepartamentosByPais(pais); generales$departamento = generales$pais + ":" + c.getCodDepartamentoDomic(); depto = sessionBeanParametros.findDepartamentoById(new
         * DeptosPK(c.getCodPaisDomic(), c.getCodDepartamentoDomic())); municipiosDomicilioSelectItemListModel = sessionBeanParametros.findMunicipiosByDepartamento(depto); generales$municipio =
         * generales$departamento + ":" + c.getCodMunicipioDomic(); generales$telefono = c.getTelefono(); generales$direccion = c.getDireccion(); // == generales$paisNacimiento = "" +
         * c.getCodPaisNacimiento(); pais = sessionBeanParametros.findPaisesByid(c.getCodPaisNacimiento()); deptosNacSelectItemListModel = sessionBeanParametros.findDepartamentosByPais(pais);
         * generales$departamentoNacimiento = generales$paisNacimiento + ":" + c.getCodDepartamentoNacim(); depto = sessionBeanParametros.findDepartamentoById(new
         * DeptosPK(c.getCodPaisNacimiento(), c.getCodDepartamentoNacim())); municipiosNacDomicilioSelectItemListModel = sessionBeanParametros.findMunicipiosByDepartamento(depto);
         * generales$municipioNacimiento = generales$departamentoNacimiento + ":" + c.getCodMunicipioNacim(); generales$fechaNacimiento = c.getFechaNac(); generales$paisNacionalidad =
         * c.getCodPaisNacionalidad().toString(); generales$grupoSanguineo = c.getTipoSangre().getTipoSangre(); // == generales$dui = c.getNumDui(); generales$nit = c.getNumNit();
         * generales$fechaExpDui = c.getFechaExpDui();
         *
         * generales$departamentoExpDui = "2:" + c.getExpedicionDui(); depto = sessionBeanParametros.findDepartamentoById(new DeptosPK(new Short("2"), new Short(c.getExpedicionDui())));
         * municipiosExpDUISelectItemListModel = sessionBeanParametros.findMunicipiosByDepartamento(depto); generales$municipioExpDui = generales$departamentoExpDui + ":" + c.getMuniExpDui();
         *
         * generales$licenciaConducir = c.getNumLicencia(); generales$pasaporte = c.getNumPasaporte(); generales$nombreISSS = c.getNomIsss(); generales$nombreNIT = c.getNomNit(); // ==
         * preparacionesAcademicasCandidato = new ArrayList<PreparacionAcademicaCandidato>(); //List<NivelesXCandidato> ln = c.getNivelesXCandidatoList(); List<NivelesXCandidato> ln =
         * reclutamientoFacade.findNivelCandidatoByCandidato(c); for (NivelesXCandidato n : ln) { PreparacionAcademicaCandidato p = new PreparacionAcademicaCandidato();
         * p.setNombreInstitucion(n.getNomInstitucion()); p.setNivelAcademico(sessionBeanParametros.findNivelAcademicoById(new NivelAcademicoPK(n.getNivelesXCandidatoPK().getCodCia(),
         * n.getNivelesXCandidatoPK().getCodNivel()))); p.setAnioIngreso(n.getAnioIngreso()); p.setAnioEgreso(n.getAnioEgreso());
         * p.setDepartamentoInstitucion(sessionBeanParametros.findDepartamentoById(new DeptosPK(n.getCodPais(), n.getCodDepto()))); preparacionesAcademicasCandidato.add(p); } // ==
         * emergencias$conyuge = c.getNombreConyuge(); emergencias$trabajo = c.getTrabajoConyuge(); emergencias$telefono = c.getTelefonoConyuge(); emergencias$condicionSalud =
         * c.getCondicionSalud(); emergencias$actividadLimitada = c.getActividadLimitada().equals("S"); emergencias$haSufridoAccidentes = c.getTieneAccidente().equals("S");
         * emergencias$tipoAccidente = c.getTipoAccidente(); emergencias$pesoActual = c.getPeso(); emergencias$estatura = c.getEstatura();
         *
         * parentescosCandidatos = new ArrayList<ParentescoCandidato>(); List<EmergenciaXCandidato> le = reclutamientoFacade.findEmergenciasByCandidato(c); for (EmergenciaXCandidato e : le) {
         * ParentescoCandidato p = new ParentescoCandidato(); p.setParentesco(sessionBeanParametros.findParentescoById(new ParentescoPK(getSessionBeanADM().getCompania().getCodCia(),
         * e.getCodParentesco()))); p.setNombre(e.getNombre()); p.setTelefono(e.getTelefono()); parentescosCandidatos.add(p); } // == experienciasLaboralesCandidato = new
         * ArrayList<ExperienciaLaboralCandidato>(); // == referenciasLaboralesCandidato = new ArrayList<ReferenciaLaboralCandidato>(); referenciasPersonalesCandidato = new
         * ArrayList<ReferenciaPersonalCandidato>(); List<Referencia> lr = reclutamientoFacade.findReferenciasByCandidato(c); for (Referencia r : lr) { if (r.getTipoReferencia() == 'L') {
         * ReferenciaLaboralCandidato rlc = new ReferenciaLaboralCandidato(); rlc.setNombre(r.getNomReferencia()); rlc.setLugarTrabajo(r.getLugar()); rlc.setTelefono(r.getTelefono());
         * rlc.setCorreoElectronico(r.getEmail()); referenciasLaboralesCandidato.add(rlc); } else { ReferenciaPersonalCandidato rpc = new ReferenciaPersonalCandidato();
         * rpc.setNombre(r.getNomReferencia()); rpc.setLugarTrabajo(r.getLugar()); rpc.setTiempoConocerle(new Integer(r.getTiempo())); rpc.setTelefono(r.getTelefono());
         * rpc.setCorreoElectronico(r.getEmail()); referenciasPersonalesCandidato.add(rpc); } } // == documentosCandidato = new ArrayList<DocumentoCandidato>(); List<DocumentoPresentado> ld =
         * reclutamientoFacade.findDocumentosByCandidato(c); for (DocumentoPresentado d : ld) { DocumentoCandidato dc = new DocumentoCandidato(); dc.setTipo(d.getTipoDocumento());
         * dc.setNumero(d.getObservacion()); documentosCandidato.add(dc); } // == capacitacionesCandidato = new ArrayList<CapacitacionCandidato>(); List<CapacitacionXCandidato> lc =
         * reclutamientoFacade.findCapacitacionesByCandidato(c); for (CapacitacionXCandidato cxc : lc) { CapacitacionCandidato cc = new CapacitacionCandidato();
         * cc.setInstitucion(cxc.getNomInstitucion()); cc.setDescripcion(cxc.getDescripcion()); cc.setPeriodo(cxc.getFecha()); cc.setTipo(cxc.getTipo()); capacitacionesCandidato.add(cc); } // ==
         * dependientesCandidato = new ArrayList<DependienteCandidato>(); List<DependienteXCandidato> ldeps = reclutamientoFacade.findDependientesByCandidato(c); for (DependienteXCandidato dxc :
         * ldeps) { DependienteCandidato dc = new DependienteCandidato(); dc.setNombre(dxc.getNombre()); dc.setParentesco(sessionBeanParametros.findParentescoById(new
         * ParentescoPK(dxc.getDependienteXCandidatoPK().getCodCia(), dxc.getCodParentesco()))); dc.setFechaNacimiento(dxc.getFechaNacimiento()); dependientesCandidato.add(dc); } // ==
         * idiomasCandidato = new ArrayList<IdiomaCandidato>(); List<IdiomaXCandidato> li = reclutamientoFacade.findIdiomasByCandidato(c); for (IdiomaXCandidato ixc : li) { IdiomaCandidato ic =
         * new IdiomaCandidato(); ic.setIdioma(sessionBeanParametros.findIdiomaById(new IdiomaPK(ixc.getIdiomaXCandidatoPK().getCodCia(), ixc.getIdiomaXCandidatoPK().getCodIdioma())));
         * ic.setNivel(new Integer(ixc.getNivel())); ic.setLee(ixc.getLee().equals("S")); ic.setEscribe(ixc.getEscribe().equals("S")); idiomasCandidato.add(ic); } // == beneficiariosCandidato =
         * new ArrayList<BeneficiarioCandidato>(); List<BeneficiarioXCandidato> lb = reclutamientoFacade.findBeneficiariosByCandidato(c); for (BeneficiarioXCandidato b : lb) {
         * BeneficiarioCandidato bc = new BeneficiarioCandidato(); bc.setNombre(b.getNombre()); bc.setParentesco(b.getParentesco()); beneficiariosCandidato.add(bc); } // == equiposCandidato = new
         * ArrayList<EquipoCandidato>(); List<Equipo> leq = c.getEquipoList(); for (Equipo eq : leq) { EquipoCandidato eqc = new EquipoCandidato(); eqc.setEquipo(eq); equiposCandidato.add(eqc); }
         * // == pruebasCandidato = new ArrayList<PruebaCandidato>(); List<TipoPruebaXCandidato> ltprueba = reclutamientoFacade.findTiposPruebasByCandidato(c); for (TipoPruebaXCandidato tp :
         * ltprueba) { PruebaCandidato pc = new PruebaCandidato(); pc.setTipoPrueba(tp.getTipoPrueba()); pc.setFecha(tp.getFecha()); pc.setNota(tp.getNota().doubleValue());
         * pc.setCosto(tp.getCosto().doubleValue()); pc.setResultado(tp.getResultado()); pruebasCandidato.add(pc); } // == puestosCandidato = new ArrayList<PuestoCandidato>();
         * List<CandidatoXCargo> lcargo = reclutamientoFacade.findCargosByCandidato(c); for (CandidatoXCargo cxc : lcargo) { PuestoCandidato pcan = new PuestoCandidato();
         * pcan.setPuesto(cxc.getPuestos()); pcan.setSalarioAspirado(cxc.getSalarioAspirado()); puestosCandidato.add(pcan); } // == entrevistasCandidato = new ArrayList<EntrevistaCandidato>();
         * List<EntrevistaXCandidato> lentr = reclutamientoFacade.findEntrevistasByCandidato(c); for (EntrevistaXCandidato entr : lentr) { EntrevistaCandidato ec = new EntrevistaCandidato();
         * ec.setPuesto(entr.getPuesto()); ec.setEntrevistador(entr.getEmpleado()); ec.setFecha(entr.getFecha()); ec.setDescripcion(entr.getDescripcion()); ec.setResultado(entr.getResultado());
         * entrevistasCandidato.add(ec); } // == observaciones = c.getObservacion();
         */
        // ==
...
private boolean guardarCandidato()
{
    try
        ...
        candidato.setEtnia(generales$etnia);
...
//        candidato.setMuniExpDui(sessionBeanParametros.findMunicipiosById(municipioExpDUI).getNomMuni());
        ...
        candidato.setNumIrtra(generales$irtra);
        candidato.setNumIsss(generales$isss);
        ...
        for (PreparacionAcademicaCandidato preparacionCandidato : preparacionesAcademicasCandidato)
            ...
            nivelCandidatoPK.setCodNivel(preparacionCandidato.getNivelAcademico().getNivelAcademicoPK().getCodNivelAcademico());
            ...
            nivelCandidato.setSubNivel(preparacionCandidato.getProfesion().getProfesionPK().getCodProfesion());
        ...
        List<ExpLaboralCandidato> listaExperiencias = reclutamientoFacade.findExperienciasLaboralesByCandidato(candidato);
        for (ExpLaboralCandidato elc : listaExperiencias)
            {
            reclutamientoFacade.eliminarExpLaboralCandidato(elc);
            }

        i = 0;
        for (ExperienciaLaboralCandidato experienciaCandidato : experienciasLaboralesCandidato)
            {
            ExpLaboralCandidatoPK expLaboralCandidatoPK = new ExpLaboralCandidatoPK();
            expLaboralCandidatoPK.setCodCia(candidato.getCandidatoPK().getCodCia());
            expLaboralCandidatoPK.setCodCandidato(candidato.getCandidatoPK().getCodCandidato());
            expLaboralCandidatoPK.setCodExpLaboral(++i);

            ExpLaboralCandidato expLaboralCandidato = new ExpLaboralCandidato(expLaboralCandidatoPK);
            expLaboralCandidato.setCandidato(candidato);
            expLaboralCandidato.setLugarTrabajo(experienciaCandidato.getLugar());
            expLaboralCandidato.setPuestos(experienciaCandidato.getPuesto());
            expLaboralCandidato.setFechaInicio(experienciaCandidato.getFechaInicio());
            expLaboralCandidato.setFechaFin(experienciaCandidato.getFechaFin());
            expLaboralCandidato.setMotivoRetiro(experienciaCandidato.getMotivoRetiro());

            reclutamientoFacade.crearExpLaboralCandidato(expLaboralCandidato);
            }
        ...
        for (EntrevistaCandidato entrevistaCandidato : entrevistasCandidato)
            ...
            entrevistaXCandidato.setNomEntrevistador(entrevistaCandidato.getNombreEntrevistador());
...
//        listaCargos.addAll(reclutamientoFacade.findCargosByCandidato(candidato));
//        for (CandidatoXCargo puestoCand : listaCargos)
//            reclutamientoFacade.eliminarPuestoCandidato(puestoCand);
//
//        for (PuestoCandidato puestoCandidato : puestosCandidato)
...
//        listaEntrevistas.addAll(reclutamientoFacade.findEntrevistasByCandidato(candidato));
//        for (EntrevistaXCandidato entrevCand : listaEntrevistas)
//            reclutamientoFacade.eliminarEntrevistaCandidato(entrevCand);
//
//        for (EntrevistaCandidato entrevistaCandidato : entrevistasCandidato)
...
//                nivelCandidatoPK.setCodNivel(new Short("" + (++i)));                
        ...
        for (ReferenciaLaboralCandidato referenciaLaboral : referenciasLaboralesCandidato)
            ...
            referencia.setNomReferencia(referenciaLaboral.getNombre());
            referencia.setLugar(referenciaLaboral.getLugarTrabajo());
            referencia.setPuesto(referenciaLaboral.getPuesto());
            referencia.setEmail(referenciaLaboral.getCorreoElectronico());
            referencia.setTelefono(referenciaLaboral.getTelefono());
        ...
        for (PuestoCandidato puestoCandidato : puestosCandidato)
            ...
            candidatoCargo.setCandidato(candidato);
            candidatoCargo.setPuestos(puestoCandidato.getPuesto());
            candidatoCargo.setCodTipoPuesto(((puestoCandidato.getPuesto() != null) && (puestoCandidato.getPuesto().getTipoPuesto()) != null) ? puestoCandidato.getPuesto().getTipoPuesto().getTipoPuestoPK().getCodTipoPuesto() : null);
            candidatoCargo.setSalarioAspirado(puestoCandidato.getSalarioAspirado());
...
private TipoDocumentoFacade lookupTipoDocumentoFacadeBean()
{
    try
        {
        Context c = new InitialContext();
        return (TipoDocumentoFacade) c.lookup("java:global/com.infosgroup.toto_PlanillaWebPRADO_war_1.0-SNAPSHOT/TipoDocumentoFacade!com.infosgroup.planilla.modelo.facades.TipoDocumentoFacade");
        }
