...
public class CandidatoFacade extends AbstractFacade<Candidato, CandidatoPK>
{
    ...
    public List<Candidato> getCandidatoConCriteriosPuesto(Concurso c, String usuario, Integer maxRegistros)
    {
        ...
        query.append("and pkg_web_PLANILLA.candidato_valido( ").append(" ? ");
        ...
        listaCandidatos = em.createNativeQuery(query.toString(), Candidato.class).setParameter(1, c.getConcursoPK().getCodCia()).setParameter(2, c.getPuestos().getPuestosPK().getCodPuesto()).setParameter(3, c.getConcursoPK().getCodConcurso()).setParameter(4, usuario).getResultList();
    ...
    {
        TypedQuery<Candidato> tq = getEntityManager().createQuery("SELECT c FROM Candidato c WHERE c.empleados = :emp", entityClass);
        tq.setParameter("emp", emp);
        List<Candidato> lc = tq.getResultList();
        return ((lc != null) && !tq.getResultList().isEmpty()) ? lc.get(0) : null;
    }
}