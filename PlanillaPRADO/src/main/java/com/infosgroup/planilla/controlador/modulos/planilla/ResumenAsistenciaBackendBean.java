...
public void init()
{
    ...
    listaEmpleados = planillaSessionBean.listaEmpleados(getSessionBeanEMP().getCompania());
    listaAgencias = planillaSessionBean.listarAgencias(getSessionBeanEMP().getCompania());
    listaDepartamentos = planillaSessionBean.findDepartamentos(getSessionBeanEMP().getCompania());
...
public AutocompleteProgramacionPlaConverter getProgramacionPlaConverter()
{
    if (tipoPlanilla != null && tipoPlanilla != -1)
        {
        programacionPlaConverter = new AutocompleteProgramacionPlaConverter(planillaSessionBean.getProgramacionPlaSinEstado(getSessionBeanEMP().getCompania().getCodCia(), tipoPlanilla));
        }
    else
        {
        programacionPlaConverter = new AutocompleteProgramacionPlaConverter(new ArrayList<ProgramacionPla>());
...
public String imprimirResumenDeAsistencia()
{
    ...
    reportesStatelessBean.generarReporteSQL(FacesContext.getCurrentInstance(), parametros, "hojaDeAsistencia", FormatoReporte.PDF);
    return null;
}
}
