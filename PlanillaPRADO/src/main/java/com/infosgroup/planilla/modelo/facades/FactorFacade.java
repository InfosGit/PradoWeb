...
public class FactorFacade extends AbstractFacade<Factor, FactorPK> {
    ...
    public List<Factor> findByPlantilla(Plantilla plantilla) {
        Query consulta = em.createNativeQuery("select f.* from factor f where (cod_cia, cod_factor) in (select distinct dp.cod_cia, dp.cod_factor from det_plantilla dp, plantilla p where dp.cod_cia = p.cod_cia and dp.cod_tipo_evaluacion = p.cod_tipo_evaluacion and dp.cod_plantilla = p.cod_plantilla and p.cod_cia = ?1 and p.cod_tipo_evaluacion = ?2 and p.cod_plantilla = ?3) order by cod_cia, cod_factor", Factor.class);
        ...
        List<Factor> l = consulta.getResultList();
        return l != null ? l : new ArrayList<Factor>();
    }
}
