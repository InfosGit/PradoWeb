...
{
    empresa = getSessionBeanEMP().getCompania().getCodCia();
    tipoPlanilla = -1;
    limpiarCampos();
}
...
public AutocompleteProgramacionPlaConverter getProgramacionPlaConverter()
{
    if (tipoPlanilla != null && tipoPlanilla != -1)
        {
        programacionPlaConverter = new AutocompleteProgramacionPlaConverter(planillaSessionBean.getProgramacionPlaByTipo(getSessionBeanEMP().getCompania().getCodCia(), tipoPlanilla));
        }
    else
        {
...
public void handleFileUpload(FileUploadEvent event)
{
    ...
    try
        ...
        Set<ResumenAsistencia> t = new HashSet<ResumenAsistencia>();
...
{
    deduccionPrestacionSeleccionada = planillaSessionBean.findDeducPrestaByPK(new DeducPrestaPK(deduccionPrestacion, getSessionBeanEMP().getCompania().getCodCia()));
...
{
    listaDeduccionPrestacion = planillaSessionBean.findDeducPrestaByCias(getSessionBeanEMP().getCompania());
...
public List<ProgramacionPla> getListaPlanillas()
{
    if (tipoPlanilla != null && tipoPlanilla != -1)
        {
        listaPlanillas = planillaSessionBean.getProgramacionPlaByTipo(getSessionBeanEMP().getCompania().getCodCia(), tipoPlanilla);
        }
...
{
    listaTipos = planillaSessionBean.listarTipos(getSessionBeanEMP().getCompania());
