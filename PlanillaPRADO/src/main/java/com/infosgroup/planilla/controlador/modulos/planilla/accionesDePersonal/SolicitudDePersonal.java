...
public List<ProgramacionPla> getListaPlanillas()
{
    if (tipoPlanilla != null && tipoPlanilla != -1)
        {
        listaPlanillas = getPlanillaSessionBean().getProgramacionPlaByTipo(getSessionBeanEMP().getCompania().getCodCia(), tipoPlanilla);
        }
...
public AccionPersonalPK getAccionPersonalPK(Cias cias, Empleados e)
{
    ...
    try
        ...
        nuevaPK.setCodTipoaccion(accionesPersonalBackendBean.getTipoAccionPersonal() == 17 ? 2 : accionesPersonalBackendBean.getTipoAccionPersonal());
...
public void guardarAccionPersonal(AccionPersonal accionPersonal)
{
    try
        ...
        if (isInRole("rrhh"))
            {//Si quien guarda la solicitud es RRHH implica que esta autorizada y se envia el correo.
        ...
        {
        addMessage("Acciones de Personal", "Excepcion guardando la acci&oacute;n de personal", TipoMensaje.ERROR_FATAL);
        Logger.getLogger(getClass().getName()).log(Level.SEVERE, "Se desencadeno la siguiente excepcion: ", e);
        }
}
...
public TipoAccion getTipoAccion()
{
    ...
    return planillaSessionBean.buscarTipoAccion(getSessionBeanEMP().getCompania().getCodCia(), accionesPersonalBackendBean.getTipoAccionPersonal());
}
