...
public String mostrarReporte$action()
{
    ...
    parametros.put("empresa", getSessionBeanEMP().getCompania().getCodCia().toString());
    parametros.put("codDpAguinaldo", "6");
    reportesBean.generarReporteSQL(FacesContext.getCurrentInstance(), parametros, "repDetPagosEmpleado", FormatoReporte.XLS);
    return null;
}
