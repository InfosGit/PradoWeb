...
public BigDecimal getTotal()
{
    ...
    try
        {
        sMinimo = (empleados.getPuestos().getSalMinimo() != null) ? empleados.getPuestos().getSalMinimo() : BigDecimal.ZERO;
        }
    catch (Exception excpt)
        {
        sMinimo = BigDecimal.ZERO;
        }
    try
        {
        total1 = prestaciones.add(sMinimo);
...
public String getPlanillaToString()
{
    try
        {
        planillaToString = "" + planillaPK.getAnio() + "--" + new SimpleDateFormat("MMMM").format(new SimpleDateFormat("MM").parse(String.valueOf(planillaPK.getMes()))).toString().toUpperCase() + "--" + planillaPK.getNumPlanilla();
        }
