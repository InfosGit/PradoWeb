...
public class DepartamentoFacade extends AbstractFacade<Departamentos, DepartamentosPK> {
    ...
    @PermitAll
    public List<Departamentos> findDepartamentosByGerencia(Cias cias, Short gerencia)
    {
        TypedQuery<Departamentos> tq = em.createQuery("SELECT d FROM Departamentos d WHERE d.departamentosPK.codCia = :codCia AND d.codGerencia = :codGerencia ORDER BY d.nomDepto ASC", Departamentos.class);
         tq.setParameter("codCia", cias.getCodCia());
         tq.setParameter("codGerencia", gerencia);
         List<Departamentos> l =tq.getResultList();
        return l != null ? l : new ArrayList<Departamentos>();
    }
}
