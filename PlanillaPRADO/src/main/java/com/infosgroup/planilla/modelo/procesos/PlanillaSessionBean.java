...
@EJB
private TiposIncapacidadFacade tipoIncapacidadFacade;
...
@PermitAll
public List<Empleados> listarJefes(Cias cia)
{
    return empleadoFacade.findByJefes(cia);
}
...
@PermitAll
@SuppressWarnings({"UnusedAssignment", "null", "ConstantConditions"})
...
@PermitAll
public Integer totalAfectadosRangoSalarios(Cias cias, BigDecimal s1, BigDecimal s2, Short pto)
{
    return empleadoFacade.totalAfectadosRangosSalario(cias, s1, s2,pto);
}
...
//METODO ACTUAL
@PermitAll
public List<Empleados> listarAfectadosRangoSalariosPuesto(Short cias, BigDecimal s1, BigDecimal s2,Short puesto)
{
    return empleadoFacade.afectadosRangoSalarioPuesto(cias, s1, s2, puesto);
}
...
@PermitAll
public List<TiposIncapacidad> findByCompania(Cias c)
{
    return tipoIncapacidadFacade.findByCompania(c);
}
...
@PermitAll
public void eliminarAccionPersonal(AccionPersonal ap)
{
    accionPersonalFacade.remove(ap);
}
}
