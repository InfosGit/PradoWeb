...
// ==
@Resource(name = "jdbc/PradoDatasource")
...
public List<DetalleReporteEvaluacion> getDetalleReporteEvaluacion(List<DetEvaluacion> detalle)
{
    ...
    for (DetEvaluacion e : eliminarRepetidos(detalle))
        ...
        if (e.getRespuesta().getNivel() != null)
            {
            det.add(new DetalleReporteEvaluacion(e.getRespuesta().getNivel(), puntajeObtenido, e.getRespuesta().getValor().intValue(), 100, puntajeObtenido * e.getRespuesta().getValor().intValue() * 100 / 100));
            }
        }
...
public void generarConstanciaSueldo(EmpleadosPK empleadosPK)
{
    ...
    generarReporteSQL(FacesContext.getCurrentInstance(), parametros, "ReportPLANILLA", FormatoReporte.PDF);        
}
}