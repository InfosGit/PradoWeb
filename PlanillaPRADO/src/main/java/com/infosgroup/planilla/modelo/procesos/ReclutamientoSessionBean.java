...
@EJB
private NivelesXEmpleadoFacade nivelesEmpleadoFacade;
@EJB
private transient ExpLaboralCandidatoFacade expLaboralCandidatoFacade;
...
{
    return empleadoFacade.toEmpleado(c, g);
}
...
@PermitAll
public void contratarCandidato(CandidatoConcurso c, Contrato contrato, String usuario, Empleados representantePatronal, String dui, Gerencia g)
{

    Empleados e = toEmpleado(c.getCandidato1(), g);
    ...
    e.setTipoContra((contrato.getTipo() != null) ? contrato.getTipo().toString() : null);
...
@PermitAll
public void contratarCandidato2(Candidato c, Puestos p, Contrato contrato, String usuario, Empleados representantePatronal, String dui, Gerencia g)
{

    Empleados e = toEmpleado(c, g);
    e.setUsuario(usuario);
    e.setDui(dui);
    e.setSexo(c.getSexo());
    e.setPuestos(p);
    ...
    e.setCodPais(c.getCodPaisNacionalidad());
    e.setFecIngreso(new Date());
    e.setTiposPlanilla(contrato.getTiposPlanilla());
    e.setTipoContra((contrato.getTipo() != null) ? contrato.getTipo().toString() : null);
    e.setTitulo(c.getProfesion() != null ? c.getProfesion().getNomProfesion() : "");
    guardarEmpleado(e);
    contrato.setCandidato(c);
    contrato.setEmpleados(e);
    contrato.setContratoPK(new ContratoPK(c.getCandidatoPK().getCodCia(), contratoFacade.max(c), c.getCandidatoPK().getCodCandidato()));
    guardarContrato(contrato);
    c.setEstado("C");
    //editarCandidatoConcurso(c);
    c.setEmpleados(e);
...
@PermitAll
public List<TipoDocumento> findAllTipoDocumento(Cias cia)
{
    return tipoDocumentoFacade.findAll();
}

@PermitAll
public List<Profesion> findAllProfesiones(Cias cia)
{
...
@PermitAll
public void editarCapacitacionXCandidato(CapacitacionXCandidato c)
{
    capacitacionXCandidatoFacade.edit(c);
}
...
@PermitAll
public void flushEmpleado()
{
    empleadoFacade.flush();
}
...
@PermitAll
public CapacitacionXCandidato findCapacitacionCandidatoByID(CapacitacionXCandidatoPK pk)
{
    return capacitacionXCandidatoFacade.find(pk);
}
...
@PermitAll
public Candidato findCandidatoByEmpleado(Empleados emp)
{
    return candidatoFacade.findCandidatoByEmpleado(emp);
}

@PermitAll
public List<NivelesXEmp> findNivelEmpleadoByEmpleado(Empleados e)
{
    return nivelesEmpleadoFacade.findByEmpleado(e);
}

@PermitAll
public Empleados getRepresentantePatronal(Cias cia)
{
    return paramPlanFacade.find(cia.getCodCia()).getEmpleados();
}

@PermitAll
public Integer maxContrato(Candidato c)
{
    return contratoFacade.max(c);
}

@PermitAll
public Integer maxExpLaboralCandidato(Candidato c)
{
    return expLaboralCandidatoFacade.max(c);
}

@PermitAll
public void crearExpLaboralCandidato(ExpLaboralCandidato elc)
{
    expLaboralCandidatoFacade.create(elc);
}

@PermitAll
public void eliminarExpLaboralCandidato(ExpLaboralCandidato elc)
{
    expLaboralCandidatoFacade.remove(elc);
}

@PermitAll
public List<ExpLaboralCandidato> findExperienciasLaboralesByCandidato(Candidato c)
{
return expLaboralCandidatoFacade.findByCandidato(c);
}
}
